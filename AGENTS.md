...
* Use `pnpm dev` to run the app locally and verify frontend changes.
* Do not use `pnpm build` for day-to-day verification; reserve it for validating production builds or CI/release checks.
* Only run `pnpm build` if explicitly asked to validate a production build or troubleshoot a build-specific issue.
* Each component gets its own file, named after the component (e.g. `user-card.tsx` for `UserCard`).
* Do not define components inline inside `App.tsx` (or other top-level entry files); import them from their own component files instead.
* Keep `App.tsx` focused on composition/routing, not component definitions or business logic.
* Follow strict TypeScript practices:
  * Avoid `any`; prefer precise types, generics, or `unknown` with narrowing.
  * Define explicit types/interfaces for props, state, and function return values where not trivially inferred.
  * Avoid non-null assertions (`!`) unless justified with a comment explaining why it's safe.
  * Prefer type-only imports (`import type { ... }`) for types to keep runtime imports minimal.
* Any test file should be under a `/test` directory
